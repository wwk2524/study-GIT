    ...
    <el-dialog title='监考员' :visible.sync="teacherDialog" width="70%" @close="$emit('update:show', false);chooseTeacher=[];chooseTeacherCopy=[]">
            <el-row>
                ...
                <el-col :span=8 class='text-right'>
                    <el-button size="medium" type="primary" @click="selectBtn">查询</el-button>
                    <el-button size="medium" type="primary" @click="resetBtn">重置</el-button>
                </el-col>
            </el-row>
            <el-table ref='multipleTable' class='examTeacherData mt30' :data="teacherData" border stripe @select='handleCurrentTeacher'>
                ...
                <el-table-column align="left" label="报名时间" prop="signUpDate" width='200px' show-overflow-tooltip sortable>
                    <template slot-scope="scope">
                        <div>{{$api.getDateAll(scope.row.signUpDate,'-')}}</div>
                    </template>
                </el-table-column>
...
export default{
  ...
  methods: {
    ...
    getTeacherData () {
      ...
      return this.$ajax({
        method: 'post',
        url: 'baseInfo/examine/getAllExaminerDetails',
        data: params
      }).then(res => {
        if (res.data.code === 1) {
    ...
    addTeacher () {
      this.$emit('addTeacher', this.chooseTeacherCopy)
    },
